...
class UserCollection {
  ...
  static async addOne(username: string, password: string, displayName: string): Promise<HydratedDocument<User>> {
    ...
    const user = new UserModel({username, password, displayName, dateJoined});
    await user.save();
    return user;
  }
   ...
   */
  static async updateOne(userId: Types.ObjectId | string, userDetails: {password?: string; username?: string, displayName?: string}): Promise<HydratedDocument<User>> {
